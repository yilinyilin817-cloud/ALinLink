...
export const useSftpBackend = () => {
  const openSftp = useCallback(async (options: ALinLinkSSHOptions) => {
  ...
  const listDrives = useCallback(async () => {
    return await ALinLinkBridge.get()?.listDrives?.() ?? [];
  }, []);
  ...
  const startStreamTransfer = useCallback(
    async (
      options: Parameters<NonNullable<ALinLinkBridge["startStreamTransfer"]>>[0],
      onProgress?: (transferred: number, total: number, speed: number) => void,
      onComplete?: () => void,
      onError?: (error: string) => void,
    ) => {
  ...
  const onTransferProgress = useCallback((transferId: string, cb: Parameters<NonNullable<ALinLinkBridge["onTransferProgress"]>>[1]) => {
    const bridge = ALinLinkBridge.get();
